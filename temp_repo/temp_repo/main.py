...
if __name__ == '__main__':
	...
	for i in range(loss.shape[1]):
	    result, pred , classification= pot_eval(min_top_score,lt, l, ls)
	...
	for i in range(loss.shape[1]):
	 ...
	 #   print('labels is',len(labels[:, i]))
	    result, pred,classification = pot_eval(min_top_score,lt, l, ls)
	    signal_classification.append(classification)
	    if isinstance(result, dict):
	       # print('its dict')
	        # Handle result if it's a dictionary
	        # Example: Convert dictionary to DataFrame with proper columns
	...
	correct_count = sum(signal_classification)
	
	# Calculate the total number of classifications
	total_count = len(signal_classification)
	
	# Calculate the correct classification rate
	if total_count > 0:
	    classification_rate = (correct_count / total_count) * 100
	else:
	    classification_rate = 0  # Avoid division by zero
